  ...
  <script>
    ...
    document.getElementById("startButton").addEventListener("click",async()=>{
      ...
      try{
        ...
        if(transcriptBox.innerText.includes("Waiting")) transcriptBox.innerHTML="";
        transcriptBox.innerHTML += "<div class='text-emerald-500 mt-2'>[Listening Started]</div>";
      }catch(err){
        isRecording=false;
        ...
        const reason=(err && err.message)?err.message:"Microphone permission denied or unavailable.";
        transcriptBox.innerHTML += "<div class='text-red-500 mt-2'>[Start failed] "+escapeHtml(reason)+"</div>";
        console.log("Start Live Listening failed:", err);
      }
    });
    ...
    function updateUI(data){
      ...
      if(risk>=75){
        ...
        if(sound && sound.getAttribute("src")) sound.play().catch(()=>playAlertTone());
        else playAlertTone();
      }
